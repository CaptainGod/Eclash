...
object ProfileProcessor {
    ...
    // Eclash: short codes (no http scheme) are auto-expanded with this prefix
    private const val SUBSCRIPTION_PREFIX = "https://1814840116.v.123pan.cn/1814840116/"

    private fun resolveSource(source: String): String {
        val lower = source.lowercase(Locale.getDefault())
        if (lower.startsWith("http://") || lower.startsWith("https://") || lower.startsWith("content://"))
            return source
        return "$SUBSCRIPTION_PREFIX$source"
    }
    ...
    suspend fun apply(context: Context, uuid: UUID, callback: IFetchObserver? = null) {
        withContext(NonCancellable) {
            processLock.withLock {
                ...
                Clash.fetchAndValid(context.processingDir, resolveSource(snapshot.source), force) {
                    try {
                        cb?.updateStatus(it)
                    } catch (e: Exception) {
                        cb = null
                ...
                profileLock.withLock {
                    if (PendingDao().queryByUUID(snapshot.uuid) == snapshot) {
                        ...
                        if (snapshot?.type == Profile.Type.Url) {
                            val resolvedSource = resolveSource(snapshot.source)
                        if (resolvedSource.startsWith("https://", true)) {
    ...
    suspend fun update(context: Context, uuid: UUID, callback: IFetchObserver?) {
        withContext(NonCancellable) {
            processLock.withLock {
                ...
                Clash.fetchAndValid(context.processingDir, resolveSource(snapshot.source), true) {
                    try {
                        cb?.updateStatus(it)
                    } catch (e: Exception) {
                        cb = null
    ...
    private fun Pending.enforceFieldValid() {
        ...
        when {
            ...
            // Allow short codes (no scheme) — they will be expanded by resolveSource()
            source.isNotEmpty() && scheme != null && scheme != "https" && scheme != "http" && scheme != "content" ->
